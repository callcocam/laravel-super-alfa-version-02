                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm"
                                        ...
                                        <tbody wire:sortable="updateTaskOrder">
                                            @if ($produtos_campanha = \Campanha\Models\ProdutosCampanha::query()->orderBy('order')->where('campanha_id', $model->id)->where('coperat_id', $categoria->id)->get())
                                                @foreach ($produtos_campanha as $produto)                                        
                                                    <tr style="cursor: move" class="py-2 bg-gray-500 pointer"
                                                        wire:sortable.item="{{ $produto->id }}"
                                                        wire:key="task-{{ $produto->id }}"
                                                        wire:sortable.handle>
                                                        <td>
                                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                                class="cursor-pointer" fill="none" viewBox="0 0 24 24"
                                                                stroke="currentColor" stroke-width="2" width="20">
