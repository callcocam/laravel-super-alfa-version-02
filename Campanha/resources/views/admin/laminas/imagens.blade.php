...
<script>
    ...
    const tituloselo{{ $idcard }} = new fabric.IText('{{$precofracionado}}', {
        top: 80,
        left: 295,
    ...
    imagemproduto{{ $idcard }}.add(tituloselo{{ $idcard }})
    const subtituloselo{{ $idcard }} = new fabric.IText('CADA 100g', {
        top: 36,
        left: 275,
        fontFamily: 'bwglennsans-black',
        fontSize: 25,
    ...
    imagemproduto{{ $idcard }}.add(subtituloselo{{ $idcard }})
    fabric.Image.fromURL("{{$bgselo}}", function (selo{{ $idcard }}) {
        selo{{ $idcard }}.scaleToHeight(90);
        selo{{ $idcard }}.set({
            top: 30,
            left: 265,
        });
