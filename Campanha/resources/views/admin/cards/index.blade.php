...
<body>
            ...
            @forelse($model->produtos->where('status','published')->sortBy('order') as $produto)
                    @php
                                         ...
                                         if($tipo_campanha == "lamina_fim_semana"){
                                           $bgcard = url('storage') . '/' . $midias_config->bg_card_promo;
                                           $bgcardapp = url('storage') . '/' . $midias_config->bg_card_app;
                                           ...
                                           $bgcard = url('storage') . '/' . $midias_config->bg_card_promo_segunda_terca;
                                           $bgcardapp = url('storage') . '/' . $midias_config->bg_card_app_segunda_terca;
                                           ...
                                           $corselo = $midias_config->cor_selo_terca_sabor;
                                            ...
                                            $bgcard = url('storage') . '/' . $midias_config->bg_card_promo_hortifruti;
                                            $bgcardapp = url('storage') . '/' . $midias_config->bg_card_app_hortifruti;
                                           ...
                                           $corselo = $midias_config->cor_selo_hortifruti;
                                            ...
                                            $bgcard = url('storage') . '/' . $midias_config->bg_card_promo_ofertas_arrasadoras;
                                            $bgcardapp = url('storage') . '/' . $midias_config->bg_card_app_ofertas_arrasadoras;
                                         ...
                                         elseif($tipo_campanha == "lamina_extra"){
                                                $bgcard = url('storage') . '/' . $midias_config->bg_card_promo_lamina_extra;
                                               $bgcardapp = url('storage') . '/' . $midias_config->bg_card_app_lamina_extra;
                                               $corpromo = $midias_config->cor_lamina_extra;
                                               $corselo = $midias_config->cor_selo_lamina_extra;
                                             }
                                         elseif($tipo_campanha == "eletro"){
                                                $bgcard = url('storage') . '/' . $midias_config->bg_card_promo_eletro;
                                               $corpromo = "#fff";
                                             }
                                            $idcard = rand(0, 99999);
                                          ...
                                          //  $produto->descricao_comercial = ucwords($produto->descricao_comercial);
                                            ...
                                            //Prepara dados para box normal
                                                ...
                                                elseif ($lencountpromocional == 3 ):
                                                   $promoreaisfontsize = 'font-size: 170px; line-height: 130px;  ';
                ...
                <div class="card" style="">
                    <div class="canvas" id="canvas{{ $idcard }}"
                         @if ($produto->app == 'sim')
                         style="background-image: url('{{ $bgcardapp }}');"
                        @else
                            style="background-image: url('{{ $bgcard }}');"
                        @endif
                    >
                        <div class="card-content">
                            @if ($produto->app == 'sim')
                                @include('campanha::admin.cards.produtoapp')
                            @else
                                @include('campanha::admin.cards.produtopromo')
                            @endif
                        </div>
                    </div>
                    <div class="row">
                        ...
                        document.querySelectorAll('.resetar{{ $idcard }}').forEach((btn) => {
                            btn.addEventListener('click', function(e) {
                                ...
                                axios.get('/sanctum/csrf-cookie').then(response => {
                                    axios.post('{{ route('api.campanhas.cards.save', $produto) }}', json).then(
                                        resp => {
                                            // console.log(resp)
                                            if (resp.status === 200) {
    ...
    <style>
        ...
        .canvas {
            ...
            display: block;
            background: #949995;
            background-image: url("{{ url('storage') . '/' . $model->bg_card }}");
