<div>
    ...
    @include('campanha::admin.encartes.fontes')
    @php
    ...
    <ul class="nav nav-pills">
        @forelse($model->coperat as $categoria)
            <li class="nav-item">
                <a class="nav-link active ms-3" href="{{ route('campanhas.encartes.produtos', compact('model'))}}?cat={{$categoria->id}}">{{$categoria->name}}</a>
            </li>
        @empty
        @endforelse
    </ul>
                ...
                <div class="conteudo-lamina" style="display: grid; grid-template-columns: auto auto auto;grid-template-rows: auto auto auto; ">
                    @forelse($produtos->sortBy('order') as $produto)
                        @php
                          ...
                          elseif($tipo_campanha == "tabloide"){
                          $bgencarte = url('storage') . '/' . $midias_config->bg_encarte;
                          $corpromo = $midias_config->cor_encarte;
                          }
...
{{--                        <div id="lamina-produto{{ $idcard }}">--}}
                            <div class="lamina-produto-content{{ $idcard}}">
                                @if($bgbox != null)
                                    ...
                                    @else
                                    @include('campanha::admin.encartes.blocos.'.$modelo)
                                @endif
                            </div>
{{--                        </div>--}}
                            ...
                            <div class="modal-dialog modal-dialog-centered modal-xl">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        @livewire("admin.encartes.produtoconfig-component", ["model"=>$produto],
                                        key(uniqid($produto)))
                                    </div>
                                </div>
                            </div>
    ...
    <style>
        ...
        .canvas {
            ...
            height: 2577px;
            display: block;
            {{--background-image: url("{{ url('encarte/bg.jpg') }}");--}}
            background-image: url("{{$bgencarte?? ''}}");
