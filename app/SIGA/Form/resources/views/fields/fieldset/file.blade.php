<fieldset>
    <div class="input-group" x-data="{}">
        ...
        <input  wire:model{{ $field->wire_model }}="file" type="file" class="{{ $file?'form-control':'d-none' }}" x-ref="{{$field->name}}">
        @if(!$file)
            <input wire:model.lazy="{{ $field->key }}"
                   {!! $field->merge(['class'=>$field->class]) !!}  @click.prevent="$refs.{{$field->name}}.click()">
        @endif
    </div>
