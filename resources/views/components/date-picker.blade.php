...
<div>
    @php
        $customConfig = config('siga.date_picker');
    ...
    <div class="{{ $theme->divClassNotInline }}" @if($inline)  @endif>
        ...
        <input id="input_{{ data_get($date, 'field') }}"
               ...
               class="iga range_input_{{ data_get($date, 'dataField') }} {{ $theme->inputClass }} {{ data_get($column, 'headerClass') }}"
               type="text"
               placeholder="{{ trans('siga.placeholders.select') }}"
               
               wire:ignore>
    </div>
    @push('head_style')
    <link rel="stylesheet" href="{{ config('siga.plugins.flat_piker.css') }}">
    @endpush
    @push('script')
    
    <script src="{{ config('siga.plugins.flat_piker.js') }}"></script>
    <script src="{{ config('siga.plugins.flat_piker.translate') }}"></script>
        <script type="application/javascript">
            flatpickr(document.getElementsByClassName('range_input_{{ data_get($date, 'dataField') }}'), {
                    mode: 'range',
                    defaultHour: 0,
                    ...@json(config('siga.plugins.flat_piker.locales.'.app()->getLocale())),
